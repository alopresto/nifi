...
class AESSensitivePropertyProviderTest extends GroovyTestCase {
    ...
    @Test
    public void testShouldNotAllowKeyChange() throws Exception {
        // Arrange
        final int MAX_KEY_SIZE = getAvailableKeySizes().max()
        AESSensitivePropertyProvider spp = new AESSensitivePropertyProvider(getKeyOfSize(MAX_KEY_SIZE))

        // Act
        def msg = shouldFail(IllegalStateException) {
            spp.setKey(getKeyOfSize(MAX_KEY_SIZE).reverse())
        }

        // Assert
        assert msg == "The key is already set and cannot be changed"
    }

    @Test
    public void testShouldDetectKeyIsSet() throws Exception {
        // Arrange
        final int MAX_KEY_SIZE = getAvailableKeySizes().max()
        final String KEY = getKeyOfSize(MAX_KEY_SIZE)
        AESSensitivePropertyProvider spp = new AESSensitivePropertyProvider()
        assert !spp.isKeySet()
        spp.setKey(KEY)

        // Act
        boolean keyIsSet = spp.isKeySet()
