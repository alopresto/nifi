...
class TlsCertificateAuthorityClientGroovyTest extends GroovyTestCase {
    ...
    void testShouldVerifyCertificateSignatureWhenSelfSigned() {
        ...
        // Make a client config which doesn't trigger any keystore generation and just has a self-signed cert and key
        TlsClientConfig clientConfig = new StandaloneConfig()
        clientConfig.setBaseDir(baseDir)
        clientConfig.setInstanceDefinitions([])
        clientConfig.setClientDns([])
        clientConfig.initDefaults()

        TlsToolkitStandalone standalone = new TlsToolkitStandalone()

        // Act
        standalone.createNifiKeystoresAndTrustStores(clientConfig)
